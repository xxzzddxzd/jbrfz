...
class GuildCommandTests(unittest.TestCase):
    ...
    def test_private_daily_batch_does_not_reuse_cooling_controller(self) -> None:
        parser = cli.build_parser()
        with tempfile.TemporaryDirectory() as temp_dir:
            db_path = Path(temp_dir) / "accounts.db"
            with AccountDB(db_path) as db:
                for mid, diamonds in (("CONTROLLER-OLD", 10), ("CONTROLLER-NEW", 900)):
                    db.upsert_state(
                        AccountState(
                            mid=mid,
                            guest_secret="secret",
            ...
            self.assertEqual(code, 0)
            current = run.call_args.args[0]
            self.assertEqual(current.controller_mid, "CONTROLLER-NEW")
            payload = json.loads(output.getvalue())
            self.assertEqual(payload["controller"]["mid"], "CONTROLLER-NEW")
            self.assertEqual(payload["controller"]["source"], "auto_replaced")
